...
def demo_day_5(bank: Bank, alice, alice_acc, processor):
    ...
    # искусственно создаём НОВЫЙ банк с "ночным" временем через
    # time_provider — так мы гарантированно попадаем в ночное окно
    # RiskAnalyzer, не завися от того, когда реально запущен скрипт
    print("\nИмитация высокорискованной операции (ночь + крупная сумма):")
    risky_bank = Bank(name="RiskBank", time_provider=lambda: datetime(2026, 1, 1, 2, 0))
